...
// import reader from 'xlsx';  // Excel Library

// import {writeFile} from "fs";
// import { Socket } from "dgram";

// let messageHistory = []; // Store messages in memory (you can use a database instead)
...
//   cors: {
//     origin: ['http://localhost:3000', 'https://9e5b-2401-4900-1c64-cd6-d130-df4a-c5c-55f3.ngrok-free.app'],
//     methods: ['GET', 'POST'],
...
//       connectSrc: ["'self'", "http://localhost:3000", "https://9e5b-2401-4900-1c64-cd6-d130-df4a-c5c-55f3.ngrok-free.app", "ws://localhost:3000"],
//     },
//   },
// }));
...
//  // console.log(sheetData[0].Command);

//   const commands = sheetData.map(row => row[Object.keys(row)[0]]).filter(command => command);
//   const delays = sheetData.map(row => row[Object.keys(row)[1]]).filter(delays => delays);
...
//   res.setHeader("Content-Security-Policy", "default-src 'self'; connect-src 'self' ws://localhost:3000 http://localhost:3000");
//   next();
// });
...
//         socket.emit("connectionstatus" , {mqttstatus:"connected",id: socket.id});
//     });
...
//   //Receiving messages from broker..
...
//           socket.emit('message',{ topic : topic2,
...
//             // **Step 1: Excel File Read Karo (Agar Nahi Hai, To Create Karo)**
//             let file;
...
//             // **Step 2: Current Date Ka Sheet Name Generate Karo**
//             const currentDate = new Date().toISOString().split('T')[0];  // Format: YYYY-MM-DD
//             const sheetName = `Sheet_${currentDate}`;
            
//             // **Step 3: Check Karo Ki Sheet Hai Ya Nahi**
//             let ws;
//             if (file.Sheets[sheetName]) {
//                 ws = file.Sheets[sheetName];  // Agar hai toh wahi use karo
//             } else {
//                 ws = reader.utils.json_to_sheet([]);  // Nahi hai toh naya sheet banao
//                 reader.utils.book_append_sheet(file, ws, sheetName);
//             }



//              // **Step 4 to 7: Append and Save Data**
//              const newData = [{
...
//     socket.emit("connectionstatus" , {mqttstatus:'disconnected',id: socket.id});
//   }
// });
...
// //Publishing the message over a topic..
// socket.on("publishmessage", (publishData) =>{
//     if(client){

//       console.log(publishData.topic1);
//       console.log(publishData.message1);
//       client.publish(publishData.topic1 , publishData.message1 , (err) => {
//           if(err){
//             console.error(`Mqtt publish error : ${err.message}`);
//           }else{
//             console.log(`Published message: "${publishData.message1}" to topic: "${publishData.topic1}"`);
         


//           // File Creation and Sheet Management**
...
//           reader.writeFile(file, filePath);
//           console.log(`Data saved to ${sheetName} in ${filePath}`);
...
//   let  R1 = RESPONSE.response1;
//    let  R2 = RESPONSE.response2;
//    let R3 = RESPONSE.response3;
